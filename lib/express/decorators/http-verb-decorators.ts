...
const executeDecorator = (fnName: string, url: string) => {
  return <T>(target: any, propertyKey: string | symbol, descriptor: any) => {
    ...
    const action = decoratorMethods.find(
      (fn: string) => fnName.toLowerCase() === fn
    );

    if (!isController(target)) {
      throw new Error(
        `[@${fnName}]: target class must be instance of Controller.`
      );
    }

    if (!validUrl(url)) {
      throw new Error(
        `[@${fnName}(${url})]: supplied url must be of type string.`
      );
    }
...
export const Get = (url: string): MethodDecorator => <T>(
  target: any,
...
export const Post = (url: string): MethodDecorator => <T>(
  target: any,
  propertyKey: string | symbol,
  descriptor: TypedPropertyDescriptor<T>
) => {
...
export const Put = (url: string): MethodDecorator => <T>(
  target: any,
  propertyKey: string | symbol,
  descriptor: TypedPropertyDescriptor<T>
) => {
...
export const Delete = (url: string): MethodDecorator => <T>(
  target: any,
  propertyKey: string | symbol,
  descriptor: TypedPropertyDescriptor<T>
) => {
